...
(function ($) {
    ...
    $(function () {
        ...
        //푸터 관련기관 사이트 시작
        var $famItem = $footer.find('.fam_list .fam_item');
        
        $famItem.each(function(){
            var $thisItem = $(this),
                $openFamBtn = $thisItem.find('button.fam_btn.open');
            
            $openFamBtn.on('click', function(){
                var $MyFamBtn = $(this),
                    $MyFamItem = $MyFamBtn.parent('.fam_item'),
                    IsActive = $MyFamItem.is('.active'),
                    $MyFamBox = $MyFamBtn.siblings('.fam_site_box');
                
                if (!IsActive){
                    $MyFamBox.slideDown(300);
                    $MyFamItem.addClass('active');
                    $MyFamBtn.attr('title', '하위 리스트 닫기');
                } else{
                    $MyFamBox.slideUp(300);
                    $MyFamItem.removeClass('active');
                    $MyFamBtn.attr('title', '하위 리스트 열기');
                }
            });
        });
        //푸터 관련기관 사이트 끝
        ...
        //푸터 스크롤탑 이동 시작
        $('.footer_bttm .top_box .top_btn').on('click', function() {
            $('html, body').animate({
